...
describe('calculateScoreV2 (Current)', () => {
  ...
  it('applies sigmoid formula with smooth decay', () => {
    // Sigmoid decays very slowly at first due to k=105
    expect(calculateScoreV2(0.5)).toBe(5000);  // Still rounds to 5000
    expect(calculateScoreV2(1)).toBe(4999);    // Barely below max
    expect(calculateScoreV2(10)).toBe(4943);   // Noticeable drop
    expect(calculateScoreV2(10)).toBeLessThan(5000);
    expect(calculateScoreV2(10)).toBeGreaterThan(4900);
  });

  it('provides better score distribution than V1', () => {
    // Actual scores with k=105, p=1.9 (verified to be better than V1)
    expect(calculateScoreV2(10)).toBeCloseTo(4943, -1);   // ~4943 pts
    expect(calculateScoreV2(50)).toBeCloseTo(4019, -1);   // ~4019 pts
    expect(calculateScoreV2(100)).toBeCloseTo(2616, -1);  // ~2616 pts (more punishing than V1)
    expect(calculateScoreV2(300)).toBeCloseTo(599, -1);   // ~599 pts (much more punishing)
    expect(calculateScoreV2(1000)).toBeCloseTo(68, -1);   // ~68 pts (much more punishing)

    // Verify it's more punishing than V1 for medium/large distances
    expect(calculateScoreV2(100)).toBeLessThan(calculateScoreV1(100));
    expect(calculateScoreV2(300)).toBeLessThan(calculateScoreV1(300));
    expect(calculateScoreV2(1000)).toBeLessThan(calculateScoreV1(1000));
  });
