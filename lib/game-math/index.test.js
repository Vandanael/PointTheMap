...
import { SCORING_FORMULA } from '../config/scoring.js';
...
describe('haversine', () => {
  ...
  it('uses shortest arc across antimeridian (170 to -170)', () => {
    /** @type {[number, number]} */
    const point1 = [0, 170];
    /** @type {[number, number]} */
    const point2 = [0, -170];
    const distance = haversine(point1, point2);
    expect(distance).toBeCloseTo(2224, -1);
  });
...
describe('calculateScoreV2 (Current)', () => {
  ...
  it('uses SCORING_FORMULA as canonical default config', () => {
    const scoreFromDefault = calculateScoreV2(123.45);
    const scoreFromFormula = calculateScoreV2(123.45, SCORING_FORMULA);
    expect(scoreFromDefault).toBe(scoreFromFormula);
  });
});
