...
const isTest =
  ...
  /** @type {any} */ (
    globalThis.__vitest__ ||
      /** @type {any} */ (globalThis).__VITEST__ ||
...
export const haversine = ([lat1, lon1], [lat2, lon2]) => {
  ...
  const [dLatRad, dLonRad] = [lat2 - lat1, dLon].map(toRad);
  const a =
 ...
 * @param {number} [config.k=100] - Scale parameter (distance at ~50% score)
 * @param {number} [config.p=2.5] - Power parameter (controls decay steepness)
 * @param {number} [config.maxScore=5000] - Maximum possible score
 * @returns {number} Score (0-5000)
 */
export const calculateScoreV2 = (distanceKm, config = {}) => {
  const { perfectThreshold = 0.1, k = 800, p = 1.2, maxScore = 5000 } = config;
