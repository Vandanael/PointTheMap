...
const defaultConfig = { enabled: true, maxBonus: 1000, maxBonusPercent: 0.2, distanceThreshold: 200 };
  ...
  it('returns 0 when maxBonus is null', () => {
    expect(
      calculateTimeBonus({
        ...
        timeRemainingMs: 5000,
        totalTimeMs: 6000,
        timeBonusConfig: { enabled: true, maxBonus: /** @type {any} */ (null), maxBonusPercent: 0.2 },
      })
    ).toBe(0);
  });
