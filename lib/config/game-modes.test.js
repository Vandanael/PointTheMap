...
describe('Game Modes Configuration', () => {
  ...
  describe('Daily selection invariants', () => {
    it('uses equivalent seeded date logic across all daily variants', () => {
      const sampleDate = new Date('2024-01-15');
      const dailySeed = assertDefined(GAME_MODES.daily.capitalSelection, 'Missing daily capitalSelection')
        .seed(sampleDate);
      const countryDailySeed = assertDefined(
        GAME_MODES.country_daily.countrySelection,
        'Missing country_daily countrySelection'
      ).seed(sampleDate);
      const stadiumDailySeed = assertDefined(
        GAME_MODES.stadium_daily.stadiumSelection,
        'Missing stadium_daily stadiumSelection'
      ).seed(sampleDate);
      const civilizationDailySeed = assertDefined(
        GAME_MODES.civilization_daily.civilizationSelection,
        'Missing civilization_daily civilizationSelection'
      ).seed(sampleDate);

      expect(countryDailySeed).toBe(dailySeed);
      expect(stadiumDailySeed).toBe(dailySeed);
      expect(civilizationDailySeed).toBe(dailySeed);
    });
  });
