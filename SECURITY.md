...
3. Copiez et exécutez le contenu du fichier `supabase-security.sql`
...
3. **Renforcez les limites** : Modifiez les constantes dans `supabase-security.sql`
...
2. **Réexécutez le script de sécurité** :
   ```sql
   -- Réexécuter supabase-security.sql
   ```
...
- `idx_leaderboard_score_desc` : Tri rapide par score
- `idx_leaderboard_pseudo` : Recherche par pseudo
- `idx_leaderboard_timestamp` : Rate limiting efficace
- `idx_leaderboard_session_id` : Suivi des sessions
...
- [ ] Script SQL `supabase-security.sql` exécuté
